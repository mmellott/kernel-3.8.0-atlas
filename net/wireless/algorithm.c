				...
				if(debug && enable_status_debug){	
					...
					printk("atlaslog");
						    ...
						    time_to_tm(t.tv_sec, 0, &broken);
   						    printk(",2013:12:%d:%d:%d:%d", broken.tm_mday, broken.tm_hour, broken.tm_min, broken.tm_sec);


					for (i=0; i<N; i++){
							  printk(",%d,%d,%d,%d", state_header_vect[i].closed, state_header_vect[i].finished, state_header_vect[i].offer, state_header_vect[i].claim);
					}
					printk("\n");
					printk("*********************************\n");	
				}	
